...
const FormSchema = z
  .object({
    ...
    secretKey: z
      .string()
      .min(6, 'Secret key is required for mentor registration'),
    ...
    email: z
      ...
      .endsWith('@am.amrita.edu', 'Must be an Amrita student email (@am.amrita.edu)'),
    password: z
      .string()
    ...
    try {
      ...
      const res = await fetch('/api/auth/register', {
        ...
        body: JSON.stringify({
          ...
          secretKey: formData.secretKey, // Include secret key for mentor registration
          email: formData.email,
          password: formData.password,
          role: 'MENTOR', // Assuming role is fixed for mentor registration
        }),
      });
      ...
      const checkSession = await fetch('/api/auth/session');
      const session = await checkSession.json();
...
{/* 
        ...
        <form className="space-y-4 md:space-y-6" onSubmit={handleSubmit(onSubmit)}>
          ...
          <div>
            <label htmlFor="secretKey" className="block mb-2 text-sm font-medium text-gray-900">
              Secret Key
            </label>
            <input
              {...register('secretKey')}
